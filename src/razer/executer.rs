...
#[rustfmt::skip]
impl<'a> Executer<'a> {
    pub fn new(device: &'a Device, app_config: &'a mut AppConfig, persist_buffer: PersistBuffer, rx: Receiver<DeviceCmd>) -> Self {
        Self { device, app_config, persist_buffer, rx, brightness_worker: BrightnessWorker::new(), display_manager: DisplayManager::new().expect("Error: No displays found"), refresh_cycle_timeout: Instant::now(), battery_cycle_timeout: Instant::now() }
    }
    pub fn process_commands(&mut self) {
        while let Ok(cmd) = self.rx.recv() { self.dispatch(cmd); }
        println!("All handles dropped. Handle thread exiting.");
    }
    fn kb(&mut self) -> KeyboardHandler<'_> { KeyboardHandler::new(self.device, self.app_config, &self.persist_buffer) }
    fn perf(&mut self) -> PerformanceHandler<'_> { PerformanceHandler::new(self.device, self.app_config, &self.persist_buffer) }
    fn display(&mut self) -> DisplayHandler<'_> { DisplayHandler::new(self.device, self.app_config, &self.persist_buffer, &mut self.display_manager, &mut self.refresh_cycle_timeout) }
    fn battery(&mut self) -> BatteryHandler<'_> { BatteryHandler::new(self.device, self.app_config, &self.persist_buffer, &mut self.battery_cycle_timeout) }
    fn dispatch(&mut self, cmd: DeviceCmd) {
        match cmd {
            DeviceCmd::InitializeDevice(n) => self.initialize(n),
            DeviceCmd::SleepDevice => self.sleep(),
            DeviceCmd::Shutdown(tx) => { let _ = tx.send(self.shutdown()); }
            ...
            DeviceCmd::AdjustScreenBrightness(change) => { self.brightness_worker.adjust_screen_brightness(change); }
            DeviceCmd::CycleRefreshRate => self.display().cycle_refresh_rate(),
            DeviceCmd::SetMuteIndicator(io, muted) => { AudioHandler::new(self.device).set_mute_indicator(io, muted); }
            DeviceCmd::CycleBatteryLimit => self.battery().cycle_battery_limit(),
            DeviceCmd::GetPID(tx) => { let _ = tx.send(self.device.info.pid); }
            DeviceCmd::GetPerfMode(tx) => { let _ = tx.send(self.perf().get_perf_mode()); }
            DeviceCmd::GetDefaultMultimediaKeys(tx) => { let _ = tx.send(self.kb().get_default_multimedia_keys()); }
            DeviceCmd::ToggleDefaultMultimediaKeys(tx) => { let _ = tx.send(self.kb().toggle_default_multimedia_keys()); }
            DeviceCmd::GetConfig(tx) => { let _ = tx.send(self.app_config.clone()); }
            DeviceCmd::PersistConfig => { crate::config::persist_config(self.app_config, &self.persist_buffer); }
        }
    }
    fn initialize(&mut self, notify_startup: bool) {
        ...
        self.brightness_worker.set_screen_brightness(state.screen_lvl);
        ...
        if self.app_config.default_multimedia_keys { self.kb().enable_multimedia_keys(); } else { self.kb().restore_fn_keys(); }
        app().send(OsdEvent::EnableOSD(true).into());
        if notify_startup { app().send(OsdEvent::Startup.into()); }
        if state.screen_refresh != 0 { self.display().set_refresh_rate(state.screen_refresh); }
    ...
    fn shutdown(&mut self) -> bool { self.kb().restore_fn_keys(); self.kb().set_rgb_effect(RGBEffect::Cycle); true }
}
